...
func (Repository) ListSubscribedPodcasts(ctx context.Context, userid int64, since time.Time,
	...
	logger := log.Ctx(ctx)
	logger.Debug().Int64("user_id", userid).Msgf("get subscribed podcasts user_id=%d since=%s", userid, since)
...
func (Repository) ListPodcasts(ctx context.Context, userid int64, since time.Time,
	...
	logger := log.Ctx(ctx)
	logger.Debug().Int64("user_id", userid).Msgf("get all podcasts user_id=%d since=%s", userid, since)
...
func (Repository) GetPodcastByID(
	...
	logger.Debug().Int64("user_id", userid).Int64("podcast_id", podcastid).
		Msgf("get podcast for user_id=%d podcast_id=%d", userid, podcastid)
...
func (Repository) GetPodcast(
	...
	logger.Debug().Int64("user_id", userid).Str("podcast_url", podcasturl).
		Msgf("get podcast for user_id=%d podcast_url=%q", userid, podcasturl)
...
func (Repository) SavePodcast(ctx context.Context, podcast *model.Podcast) (int64, error) {
	...
	if podcast.ID == 0 {
		logger.Debug().Object("podcast", podcast).
			Msgf("insert podcast user_id=%d podcast_url=%q", podcast.User.ID, podcast.URL)
		...
		logger.Debug().Object("podcast", podcast).
			Msgf("podcast created user_id=%d podcast_id=%d, podcast_url=%q", podcast.User.ID, id, podcast.URL)
	...
	// update
	logger.Debug().Object("podcast", podcast).
		Msgf("update podcast user_id=%d podcast_id=%d podcast_url=%q", podcast.User.ID, podcast.ID, podcast.URL)
...
func (Repository) UpdatePodcastsInfo(ctx context.Context, update *model.PodcastMetaUpdate) error {
	...
	logger.Debug().Object("update", update).Msgf("update podcast info podcast_url=%q", update.URL)
...
func (Repository) DeletePodcast(ctx context.Context, podcastid int64) error {
	...
	logger.Debug().Int64("podcast_id", podcastid).Msgf("delete podcast podcast_id=%d", podcastid)
